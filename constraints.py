...
def jac_of_constraint(x, *args):
    ...
    if args[0] == 'model':
        ...
        if args[1][0] == 'x1':
            jac_cons[0, i] = -1
            jac_cons[0, i + 1] = 1
            jac_cons[2, i] = p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(1/2)*(theta[i+1] - theta[i]) + p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(-1)
            jac_cons[2, i + 1] = p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(1/2)*(theta[i+1] - theta[i]) + p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)
            jac_cons[3, i] = (v1[i]/2 + v1[i+1]/2)*np.sin(theta1[i]/2 + theta1[i+1]/2)*(1/2)*p.dt
            jac_cons[3, i+1] = (v1[i]/2 + v1[i+1]/2)*np.sin(theta1[i]/2 + theta1[i+1]/2)*(1/2)*p.dt
            jac_cons[7, i] = -(1/2)*np.cos(theta1[i]/2 + theta1[i+1]/2)*p.dt
            jac_cons[7, i+1] = -(1/2)*np.cos(theta1[i]/2 + theta1[i+1]/2)*p.dt
        
        elif args[1][0] == 'y1':
            jac_cons[1, i] = -1
            jac_cons[1, i + 1] = 1
            jac_cons[2, i] = -p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(-1)*(1/2)*(theta[i+1] - theta[i]) - p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(-1)
            jac_cons[2, i + 1] = -p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(-1)*(1/2)*(theta[i+1] - theta[i]) - p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)
            jac_cons[3, i] = -(v1[i]/2 + v1[i+1]/2)*np.cos(theta1[i]/2 + theta1[i+1]/2)*(1/2)*p.dt
            jac_cons[3, i + 1] = -(v1[i]/2 + v1[i+1]/2)*np.cos(theta1[i]/2 + theta1[i+1]/2)*(1/2)*p.dt
            jac_cons[7, i] = -(1/2)*np.sin(theta1[i]/2 + theta1[i+1]/2)*p.dt
            jac_cons[7, i + 1] = -(1/2)*np.sin(theta1[i]/2 + theta1[i+1]/2)*p.dt
            
        elif args[1][0] == 'theta1':
            jac_cons[3, i] = -1
            jac_cons[3, i+1] = 1
            jac_cons[5, i] = -(1/2)*p.dt
            jac_cons[5, i + 1] = -(1/2)*p.dt
            ...
            jac_cons[2, i] = -p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(1/2)*(theta[i+1] - theta[i]) - p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(-1)
            jac_cons[2, i + 1] = -p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(1/2)*(theta[i+1] - theta[i]) - p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)
            jac_cons[4, i] = (v2[i]/2 + v2[i+1]/2)*np.sin(theta2[i]/2 + theta2[i+1]/2)*(1/2)*p.dt
            jac_cons[4, i + 1] = (v2[i]/2 + v2[i+1]/2)*np.sin(theta2[i]/2 + theta2[i+1]/2)*(1/2)*p.dt
            jac_cons[8, i] = -(1/2)*np.cos(theta2[i]/2 + theta2[i+1]/2)*p.dt
            jac_cons[8, i + 1] = -(1/2)*np.cos(theta2[i]/2 + theta2[i+1]/2)*p.dt
            ...
            jac_cons[2, i] = p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(-1)*(1/2)*(theta[i+1] - theta[i]) + p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(-1)
            jac_cons[2, i + 1] = p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(-1)*(1/2)*(theta[i+1] - theta[i]) + p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)
            jac_cons[4, i] = -(v2[i]/2 + v2[i+1]/2)*np.cos(theta2[i]/2 + theta2[i+1]/2)*(1/2)*p.dt
            jac_cons[4, i + 1] = -(v2[i]/2 + v2[i+1]/2)*np.cos(theta2[i]/2 + theta2[i+1]/2)*(1/2)*p.dt
            jac_cons[8, i] = -(1/2)*np.sin(theta2[i]/2 + theta2[i+1]/2)*p.dt
            jac_cons[8, i + 1] = -(1/2)*np.sin(theta2[i]/2 + theta2[i+1]/2)*p.dt
            
        elif args[1][0] == 'theta2':
            jac_cons[4, i] = -1
            jac_cons[4, i +1 ] = 1
            jac_cons[6, i] = -(1/2)*p.dt
            jac_cons[6, i + 1] = -(1/2)*p.dt
...
def constraint(x, *args):
    ...
    if args[0] == 'model':
        i = args[1][1]
        if args[1][0] == 'x1':
            value = x[i+1] - x[i]  + p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(theta[i+1] - theta[i]) - (v1[i]/2 + v1[i+1]/2)*np.cos(theta1[i]/2 + theta1[i+1]/2)*p.dt
        
        elif args[1][0] == 'y1':
            value = y[i+1] - y[i]  - p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(theta[i+1] - theta[i]) - (v1[i]/2 + v1[i+1]/2)*np.sin(theta1[i]/2 + theta1[i+1]/2)*p.dt
            
        elif args[1][0] == 'theta1':
            value = theta1[i+1] - theta1[i] - (omega1[i]/2 + omega1[i+1]/2)*p.dt
            
        elif args[1][0] == 'x2':
            value = x[i+1] - x[i] - p.d2/2*np.sin(theta[i]/2 + theta[i+1]/2)*(theta[i+1] - theta[i]) - (v2[i]/2 + v2[i+1]/2)*np.cos(theta2[i]/2 + theta2[i+1]/2)*p.dt
        
        elif args[1][0] == 'y2':
            value = y[i+1] - y[i] + p.d2/2*np.cos(theta[i]/2 + theta[i+1]/2)*(theta[i+1] - theta[i]) - (v2[i]/2 + v2[i+1]/2)*np.sin(theta2[i]/2 + theta2[i+1]/2)*p.dt
            
        elif args[1][0] == 'theta2':
            value = theta2[i+1] - theta2[i] - (omega2[i]/2 + omega2[i+1]/2)*p.dt
